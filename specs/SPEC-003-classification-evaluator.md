...
3. Click → el composition root construye config + cliente + evaluador (`PlatformConfig.from_env()`, `TokenProvider`, `RemoteAgentClient`) e invoca el use-case `application.run_one(case, client, evaluator)` ([ADR-005](../docs/ARCHITECTURE.md)). El dashboard **no** orquesta los pasos del protocolo (send → wait → get_final_response → evaluate): eso vive en `run_one`, la misma ruta que usa el modo batch.
