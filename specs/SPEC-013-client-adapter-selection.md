...
**Estado:** draft (implementada 2026-07-03; SC-001..003 confirmados con tests; pasa a active al tildar SC-004 —prueba funcional manual del usuario—)
...
- [ ] **SC-004**: Prueba funcional manual del usuario, condición de cierre de la spec: (a) un caso real enviado con `AGENT_CLIENT_TYPE=sync_http` contra la plataforma alternativa devuelve un veredicto correcto por el circuito completo (dashboard o runner); (b) el camino por defecto (sin `AGENT_CLIENT_TYPE`) sigue operando contra el proveedor original. Se tilda solo con el OK explícito del usuario.
