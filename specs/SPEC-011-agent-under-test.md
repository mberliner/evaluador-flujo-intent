...
**Resumen:** Introduce el **perfil de agente bajo prueba** (`AGENT_PROFILE`: `clasificador` | `traductor`, default `clasificador`): un registro en `application/` compone por perfil el `agent_id`, el constructor de entrada y el evaluador, reutilizando el mismo circuito de envío/persistencia/render. Define las abstracciones que lo habilitan: puerto `Evaluator`, supertipo `EvaluatedResult` y value object `AgentInput` (variantes estructurada|texto). Una User Story (P1), prerequisito de [[SPEC-012-translation-evaluator]]. En `draft`, pendiente de implementación.
...
- **FR-002**: MUST: Existe un **registro de perfiles** en **`src/application/`** que compone una pieza de `build/` (constructor de entrada) con una pieza de `domain/` (evaluador). El registro es **puro**: recibe el `profile_id` activo como dato (un string) y no lee `os.environ` ni importa `adapters/`; la lectura del entorno vive en `adapters/platform_config.py` (FR-004) y el composition root (`runner.py` / `dashboard/`) cablea ambos.
  > Vive en `application/` (capa de composición que ya importa de `build/` y `domain/`), no en `domain/`; la pureza preserva la regla `application ↛ adapters` (Principio II).
...
- **FR-009**: MUST: invariante [[SPEC-000-naming]] — los nombres de perfil describen *qué hace* el agente (clasificador, traductor), no la tecnología.
...
- **FR-011**: MUST: Se define el **puerto `Evaluator`** en `src/domain/ports.py` (`typing.Protocol`, consistente con los puertos existentes) que retorna el supertipo `EvaluatedResult` (FR-015), **no `Any`**. Los use-cases de `application/` y los composition roots dependen de este puerto y de ese supertipo, no del evaluador ni del resultado concreto. El `ClassificationEvaluator` existente pasa a cumplir el puerto sin heredar de él.
  > Retornar un tipo concreto (no `Any`) permite que `mypy --strict` capture incompatibilidades en compilación, no en runtime.
...
- **FR-014**: MUST: El **constructor de entrada** de cada perfil es un callable `(case) -> AgentInput`, donde **`AgentInput`** es un value object de `domain/` con dos variantes: **estructurada** (lleva el `{form}` como dato, p. ej. el clasificador) y **texto** (lleva texto natural, p. ej. el traductor). El puerto `AgentClient.send` recibe un `AgentInput` y lo renderiza al payload del proveedor según la variante (la estructurada se serializa a JSON; la de texto se envía como contenido crudo); la serialización y el conocimiento del formato del proveedor permanecen en el adapter (ADR-001; `build/` sigue puro).
  > Impacto en otras specs, a reconciliar en sus Historiales al implementar: modifica la firma de `AgentClient.send` (hoy `send(form: dict)`) → [[SPEC-002-agent-client]]; cambia el retorno de `message_builder.build` (hoy `dict` → `AgentInput` estructurada) → [[SPEC-002b-message-builder]].
...
| SC-001, SC-002, SC-003 | tests de resolución de perfil y de configuración (sin invocar al agente) |
...
- **2026-07-05** — Reescritura editorial al formato compacto (convenciones de `docs/SPEC-FORMAT.md`): Resumen ejecutivo, justificaciones movidas a notas `>` (FR-002/011/014), invariante de naming a una línea (FR-009). **Sin cambio normativo**: IDs de FR/SC y su semántica intactos.
