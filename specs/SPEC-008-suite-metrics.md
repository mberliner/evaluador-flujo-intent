...
- **FR-008**: MUST: El runner headless (`python -m src.runner`) ofrece un **modo exclusivo** (`--estadistica`) que, sin ejecutar la suite ni invocar al agente, computa la matriz general (FR-007) sobre las corridas guardadas en `--out` y la entrega en **dos formatos a la vez**: (a) **a pantalla en Markdown** (tablas alineadas legibles a simple vista: título + matriz + resumen de estadística + accuracy por clase), vía `format_metrics_markdown`; (b) **a archivo CSV** delimitado por `;` en `runs/stats/estadistica-matriz.csv`, vía el caso de uso `generate_metrics_report` (ver FR-010). Ambos formateadores son funciones puras. La salida a pantalla evita caracteres fuera de cp1252 por compatibilidad de consola en Windows. Ningún identificador nombra el formato de serialización (SPEC-000-naming).

- **FR-010**: MUST: El control del dashboard que regenera `estadistica-corridas.csv` ([[SPEC-006-batch-suite]] FR-US2-003) también actualiza `estadistica-matriz.csv` en la misma operación. La orquestación vive en el caso de uso `generate_metrics_report` (`application/generate_metrics_report.py`): recibe el repositorio, llama a `aggregate_suite_metrics` (domain), `format_metrics_report` (domain, movida desde `runner.py`), y `repo.save_metrics_report` (adapter). Tanto el dashboard como el runner headless usan el mismo caso de uso. Ambos archivos quedan sincronizados tras cada disparo del control.
...
| FR-008 | `format_metrics_report` (puro, en `domain/metrics.py`) + `format_metrics_markdown` (puro, en `runner.py`) + caso de uso `generate_metrics_report` (application) + `--estadistica` en `src/runner.py` + `save_metrics_report` en `adapters` + tests + verificación funcional CLI |
| FR-010 | caso de uso `generate_metrics_report` en `src/application/generate_metrics_report.py`; llamado desde `_render_run_stats_control` en `src/dashboard/app.py` y desde `_report_total_metrics` en `src/runner.py`. El título del reporte total tiene redacción única en `total_metrics_title` (mismo módulo, SSOT); ambos composition roots lo consumen |
...
- **2026-07-07** — Deduplicación SSOT (FR-010): el título del reporte total («Matriz de confusión — total (N corrida(s), M caso(s))») se construía con redacción idéntica en `runner.py` y `dashboard/app.py`. Se centralizó en `total_metrics_title(runs)` dentro de `application/generate_metrics_report.py`; ambos composition roots lo consumen. Sin cambio de comportamiento.
