...
- **FR-007**: MUST: Cuando el archivo no incluye `clasificacion_esperada` (caso típico de los archivos en formato puro del agente, donde el ground truth no es parte del payload), el dashboard la solicita al usuario —selectbox con `PALETA_CLASIFICACION`— y la inyecta antes de construir el `TestCase`. La carga no se habilita hasta que el usuario elige una clasificación válida. `clasificacion_esperada` no se infiere ni se deja vacía. La **detección** de la ausencia y la **inyección** viven en `src/build/case_loader.py` (`needs_expected_classification` / `with_expected_classification`), no en `dashboard/`: son conocimiento del formato del archivo, misma capa que el parseo (FR-003); el dashboard solo traduce el widget (selectbox) e invoca esas funciones.
...
| FR-007 | detección/inyección en `src/build/case_loader.py` (`needs_expected_classification` / `with_expected_classification`), solicitud por selectbox en `src/dashboard/app.py`; ejercitado **directamente** por `tests/unit/test_expected_classification.py` (detección de ausencia, inyección y round-trip end-to-end); las fixtures en `test_case_loader.py` invocan la `with_expected_classification` real; verificación funcional SC-004 |
...
- **2026-07-07** — Refactor de capas (FR-007): la detección de `clasificacion_esperada` ausente y su inyección se movieron de `src/dashboard/app.py` (`_file_needs_clasificacion` / `_inject_clasificacion`) a `src/build/case_loader.py` (`needs_expected_classification` / `with_expected_classification`) — el conocimiento del formato del archivo pertenece a `build/` (FR-003), no a la UI. El dashboard conserva solo el widget de solicitud. Tests renombrados: `test_dashboard_file_load.py` → `test_expected_classification.py`, ahora contra las funciones de `build/`. FR-007 y Coverage mapping actualizados. Sin cambio de comportamiento.
