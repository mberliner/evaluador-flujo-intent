...
1. Corre `bash tools/pipeline_local.sh` y asegúrate que esté verde (constitución, lint, format, mypy, naming, capas, bandit, pytest unit). Si el pipeline no está disponible, corre `pre-commit run --all-files` como mínimo.
