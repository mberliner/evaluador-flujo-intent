...
- **Bootstrap automático de la instalación (desde 2026-07-07)**: `tools/bootstrap_hooks.py` es el **paso 0 del pipeline local** (`step "hooks git instalados"`). Verifica primero si `pre-commit` y `post-commit` ya están en el directorio de hooks (resuelto vía `git rev-parse --git-path hooks`); si están, no toca nada; si faltan, los instala con `sys.executable -m pre_commit install --hook-type ...`. Como el protocolo obliga a correr el pipeline en cada cierre de iteración, un clon nuevo queda reparado a más tardar en su primer pipeline — antes del primer commit. Sin git es no-op (el gate funciona sin git por diseño); sin el paquete `pre-commit` falla en rojo con instrucción accionable.
