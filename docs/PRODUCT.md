...
### Modo simple (Iter 1 — primero)

1. Usuario carga **un caso por pantalla** (formulario en el dashboard).
2. La suite lo envía al agente bajo test (vía `RemoteAgentClient`).
...
| Matriz de confusión | 5×6: filas = 5 clases de `PALETA_CLASIFICACION` (Verde, Amarillo, Rojo, Negro, Rechazado); columnas = esas 5 + `Sin clasificación` (predicted vacío) | Reportada cada run |
