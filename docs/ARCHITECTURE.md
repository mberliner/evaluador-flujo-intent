...
Aplicada transversalmente. Detalle en `specs/SPEC-000-naming.md`. **El `RemoteAgentClient` usa Watson Orchestrate**: el endpoint, el formato del payload y el flujo de auth están confinados a `adapters/remote_agent_client.py` + `adapters/token_provider.py`. Cambiar de proveedor implica reescribir esos dos archivos y la spec `SPEC-002-agent-client`, nada más.
